 ...
 *   2. VITE_CS_GIST_TOKEN / VITE_CS_GIST_ID (build-time env, cheatsheets-specific)
 *
 * Intentionally separate from VITE_GIST_TOKEN / VITE_GIST_ID
 * which are reserved for the admin page Gist integration.
 */
export function getStorageCfg() {
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE_CFG) || '{}')
    const envToken = import.meta.env.VITE_CS_GIST_TOKEN ?? ''
    const envId    = import.meta.env.VITE_CS_GIST_ID    ?? ''
    return {
      // Default to gist mode if env vars are present, otherwise download
    ...
    return {
      ...
      gistToken: import.meta.env.VITE_CS_GIST_TOKEN ?? '',
      gistId:    import.meta.env.VITE_CS_GIST_ID    ?? '',
      gdriveToken: '',
      gdriveFolderId: '',
    }
  }
...
export async function saveSheet(sheet) {
  ...
  saveDraft(sheet) // always keep local draft regardless of backend
...
// ── GitHub Gist ─────────────────────────────────────────────────
/**
 * Uses VITE_CS_GIST_TOKEN / VITE_CS_GIST_ID — completely separate
 * from VITE_GIST_TOKEN / VITE_GIST_ID used by the admin page.
 *
 * On first save with no gistId configured, creates a new private Gist
 * and persists the returned ID to localStorage for all future saves.
 */
async function saveToGist(sheet, cfg) {
  const { gistToken, gistId } = cfg
  if (!gistToken) return {
    ok: false,
    message: 'No Gist token. Add VITE_CS_GIST_TOKEN to repo secrets or paste it in Settings.',
  }
  ...
  try {
    ...
    // Persist newly created Gist ID so future saves patch the same Gist
    if (!gistId) {
      setStorageCfg({ ...cfg, gistId: data.id })
    }
