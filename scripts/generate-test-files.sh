...
# TXT
echo "$CONTENT" > "$OUTDIR/test-file.txt"

# Markdown
echo -e "# Client Document Portal — Test File\n\n\`\`\`\n$CONTENT\n\`\`\`" > "$OUTDIR/test-file.md"

# HTML
cat > "$OUTDIR/test-file.html" <<EOF
<!DOCTYPE html>
<html>
<head>
<style>
...
# Placeholder PNG/JPG
echo "TEST FILE — NOT FOR PRODUCTION" > "$OUTDIR/test-file.png"
echo "TEST FILE — NOT FOR PRODUCTION" > "$OUTDIR/test-file.jpg"
